 ...
 class Explorer extends Base {
  ...
  static init(node) {
    ...
    return new Explorer({
      ...
      domain: config.str('explorer-http-domain'),
  ...
  initRouter() {
    ...
    this.get('/', async (req, res) => {
      ...
      template.append({
        ...
        module: 'block',
        category: 'latest',
        data: await this.parser.blockLatest({
          port: this.config.port,
          domain: this.config.domain || this.config.host
        })
      });
    ...
    this.get('/latest/:max', async (req, res) => {
      ...
      const blocks = await this.parser.blockLatest({
          port: this.config.port,
          domain: this.config.domain || this.config.host
        }, max);
...
class Parser {
  ...
  async blockLatest(options, max = 10) {
